...
## User goals
- integrate Kalshi API to be able to be no on player props - currently can't on website.
- SDKs (official: kalshi-python-sync and kalshi-python-async) (non-official: PyKalshi)
- Multivariate / Multi-leg endpoints (?)
- way to more accurately match each team's depth and injuries, some are wrong sometimes, affecting the player prop tab as well as calculated edge and percentages. ex(maxey who is injured is said to be playing 3/17/26 on parlay lab)
- way to scrape data in specific websites without using claude chrome controller, as it uses too much credits
- what are pandas dataframes for data analysis? pulling candlestick data into it? applicable for stock trading?

#Gemini-provided python code:
...
## 2. Load your private key file
#with open("path/to/your/private_key.pem", "r") as f:
...
#print(f"Current Balance: ${balance.balance / 100}") # Kalshi uses cents